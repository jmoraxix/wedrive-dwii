const PuntoCarga = require('../models/PuntoCarga');
...
create = async(req, res) => {
    ...
    const puntoCarga = new PuntoCarga(body);
...
updateById = async(req, res) => {
    ...
    PuntoCarga.findOneAndUpdate({ _id: req.params.id }, req.body)
        .then(() => res.status(200).json({ success: true, message: 'Objeto actualizado con exito' }))
        .catch(err => res.status(500).json({ success: false, error: err }))
}

deleteById = async (req, res) => {
    await PuntoCarga.findOneAndDelete({ _id: req.params.id })
        .then(obj => { 
            if (!obj)
                return res
                    .status(404)
...
getById = async (req, res) => {
    await PuntoCarga.findOne({ _id: req.params.id })
        .then(obj => { 
            if (!obj)
                return res
                    .status(404)
